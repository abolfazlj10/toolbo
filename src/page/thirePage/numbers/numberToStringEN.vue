...
export default {
    ...
    methods:{
        async convert (){
            if(this.num != null && this.num != "" ){
                const url = `https://api.funtranslations.com/translate/numbers.json?text=${this.num}`
                const send = await fetch(url)
                const data = await send.json()
                this.result = data.contents.translated
    ...
    created (){
        document.title='تولبو | تبدیل اعداد به  حروف انگلیسی '
        this.db.push(dbG.dataBox[2].item[5])
        this.db.push(dbG.dataBox[2].item[6])
        this.db.push(dbG.dataBox[2].item[7])
        setTimeout(() => {
            this.isShow = true
        }, 100);
    },
...
<template>
    ...
    <div v-show="isShow">
        <navs/>
        <locations title="تبدیل اعداد به  حروف انگلیسی" PN="اعداد" PNR="/numbers"/>
        <div class="container_m">
            <div class="c_tools">
            <div class="tool" :class="[(!showResult ? 'HTool' : ''),(dark ? 'tool_D' : '')]">
                <div class="title_tool">تبدیل اعداد به  حروف انگلیسی</div>
                <div class="inputs">
                    <div>عدد :</div>
