...
export default {
    ...
    methods:{
        convert (){
            if(this.percent != null && this.number != null && this.percent != "" && this.number != "" ){
                ...
                this.increase = Number(this.number) + this.result
            }else{
                this.shower()
            }
        },
    ...
    created (){
        document.title='تولبو | محاسبه افزایش درصد  '
...
<template>
    ...
    <div v-show="isShow">
        <navs/>
        <locations title="محاسبه افزایش درصد" PN="اعداد" PNR="/numbers"/>
        <div class="container_m">
            <div class="c_tools">
            <div class="tool" :class="[(!showResult ? 'HTool' : ''),(dark ? 'tool_D' : '')]">
                <div class="title_tool">محاسبه افزایش درصد </div>
                <div class="inputs">
                    <div>
                        <div>درصد افزایش%:</div>
                ...
                <div class="result" :class="dark ? 'Result_D':''" v-show="showResult">
                    ...
                    <div class="textResult">
                        <div>با {{percent}}% افزایش {{number}} می شود : <span class="colorC">{{increase}}</span> </div>
                        <div>میزان افزایش : <span class="colorC">{{result}}</span></div>
                    </div>
                </div>
            </div>
            <sidePage/>
