...
export default {
    ...
    methods:{
        convert (){
            if(this.percent != null && this.number != null && this.percent != "" && this.number != "" ){
                ...
                this.result = this.number * (this.percent / 100)
    ...
    created (){
        document.title='تولبو | محاسبه درصد '
...
<template>
    ...
    <div v-show="isShow">
        <navs/>
        <locations title="محاسبه درصد" PN="اعداد" PNR="/numbers"/>
        <div class="container_m">
            <div class="c_tools">
            <div class="tool" :class="[(!showResult ? 'HTool' : ''),(dark ? 'tool_D' : '')]">
                <div class="title_tool">محاسبه درصد</div>
                <div class="inputs">
                    <div>
                        <div>درصد%:</div>
                ...
                <div class="result" :class="dark ? 'Result_D':''" v-show="showResult">
                    ...
                    <div class="textResult">
                        {{percent}}% عدد {{number}} = <span class="colorC">{{result}}</span>
                    </div>
                </div>
            </div>
            <sidePage/>
