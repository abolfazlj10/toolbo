...
export default {
    data (){
        return{
            ...
            resultSearchNumber:{bank:null,card:null},
            searchNameBank:null,
            showSearchBox:false,
    ...
    methods:{
        checkNumber(){
            if(this.searchNumberCard.length != 6)
                this.resultSearchNumber = {bank:null,card:null}
            else if(this.searchNumberCard.length == 6 && this.resultSearchNumber.bank == null)
                this.resultSearchNumber = {bank:'err',card:null}
        },
        showResultNumber(){
            if(this.searchNumberCard != null && this.searchNumberCard.length == 6){
                this.banks.forEach((bank)=>{
                    if(bank.card == Number(this.searchNumberCard)){
                        this.resultSearchNumber = bank
                    }
                })
            }
        },
        showHelp(){
            if(this.searchNameBank.length !=0){
                this.resultSearchBank = this.banks.filter((item)=>{
                    return item.bank.search(this.searchNameBank) == 0
                })
        ...
        btnSearch (){
            var check = this.banks.filter((item)=>{
                  return item.bank.search(this.searchNameBank) == 0
            })
            if(check.length == 1){
    ...
    created (){
        document.title='تولبو | جتسجوی شماره کارت بانکی '
        this.resultSearchBank = this.banks
...
<template>
    ...
    <div v-show="isShow" @click="hiddeonBox">
        <navs/>
        <locations title="جستجو شماره کارت بانکی" PN="مالی" PNR="/fiscal"/>
        <div class="container_m">
            <div class="c_tools">
            <div class="tool" :class="dark ? 'tool_D' : ''">
                <div class="title_tool">جستجو شماره کارت بانکی </div>
                <div class="navigation">
                    <div class="itemNav" :class="type=='number'?'selected':''" @click="type='number'">پیدا کردن نام بانک براساس شماره کارت</div>
                    <div class="itemNav" :class="type!='number'?'selected':''" @click="type='bank'">پیدا کردن شماره کارت بانک</div>
                </div>
                <div class="inputs" v-show="type == 'number'">
                    <div class="label">
                        <div>شش رقم اول کارت :</div>
                        <div><input type="text" placeholder="به طور مثال : 603799" v-model="searchNumberCard" @keydown="checkNumber"></div>
                    </div>
                ...
                <div class="inputs" v-show="type != 'number'">
                    <div class="label">
                        ...
                        <div class="c_complate">
                            <input class="searchNameBank" type="text" placeholder="نام بانک مورد نظر را وارد کنید." @keypress.enter="btnSearch" v-model="searchNameBank" @focus="showSearchBox ? showSearchBox = false : showSearchBox = true">
                            <div class="complate" :class="dark ? 'complate_D':''" v-show="showSearchBox">
                                <div class="item" :class="dark?'item_D':''" v-for="banks in resultSearchBank" @click="selectBank">{{banks.bank}}</div>
                            </div>
                        </div>
                    </div>
                    <button class="bn632-hover" @click="btnSearch"><span class="fontIcon">&#xe906;</span>جستجو </button>
                </div>
                <div class="result" :class="dark ? 'Result_D':''" v-show="resultSearchNumber.bank != null && resultSearchNumber.bank != 'err'&& type == 'number'">
                    <span>نتیجه : </span>
                    <span class="colorCustom">{{resultSearchNumber.bank}}</span>
                </div>
                
                <div class="result" :class="dark ? 'Result_D':''" v-show="resultSearchNumber.bank == 'err'&& type == 'number'">
                    <span>نتیجه : </span>
                ...
                <div class="result" :class="dark ? 'Result_D':''" v-if="type != 'number' && resultBox">
                    <span>نتیجه : </span>
                    <span class="colorCustom" >{{resultSearchBank[0].card}}</span>
                </div>
            </div>
            <sidePage/>
            <usageSide :db="db"/>
